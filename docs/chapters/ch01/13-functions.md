...
参数传递的语义与初始化的语义完全相同（[§3.4.1](../ch03/34-parameters.md#3.4.1)）。也就是说，参数类型会被检查，并且必要时会进行隐式类型转换（[§1.4](../ch01/14-types-variables.md)）。例如：
...
函数可以是类的成员（[§2.3](../ch02/23-class.md)、[§5.2.1](../ch05/52-concrete-types.md)）。对于这种*成员函数*，其类名也是函数类型的一部分。例如：
...
定义多个同名函数被称为**函数重载**，是泛型编程[（§8.2）](../ch08/82-concepts.md)的核心部分之一。当一个函数被重载时，每个同名函数应该实现相同的语义。`print()` 函数就是这样的例子：每个 `print()` 都打印它的参数。
