...
异常安全（exception safety）是 C++ 资源管理的核心概念。基本保证是：当异常被抛出时，程序不会泄漏资源。这通常通过 RAII（Resource Acquisition Is Initialization，资源获取即初始化）技术来实现（[§6.3](../ch06/63-resource-mgmt.md)）。
