...
如果条件为假，编译器会产生一条包含指定消息的错误。`static_assert` 是泛型编程中非常有用的工具（[§8.2](../ch08/82-concepts.md)）。
...
如果 `noexcept` 函数实际抛出了异常，程序将调用 `std::terminate()`。`noexcept` 对于性能优化和编写异常安全的代码非常重要（[§6.2.2](../ch06/62-copy-move.md)）。
