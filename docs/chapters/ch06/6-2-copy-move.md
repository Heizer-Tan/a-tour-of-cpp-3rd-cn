...
默认情况下，对象可以被拷贝。这对用户定义类型和内置类型都成立。拷贝的默认含义是逐成员拷贝：拷贝每个成员。例如，使用 [§5.2.1](../ch05/5-2-concrete-types.md#521-算术类型) 中的 `complex`：
...
当一个类是资源句柄——即该类负责通过指针访问的对象——时，默认的逐成员拷贝通常是一场灾难。逐成员拷贝会违反资源句柄的不变式（[§4.3](../ch04/4-3-invariants.md)）。例如，默认拷贝会使 `Vector` 的副本指向与原始对象相同的元素：
