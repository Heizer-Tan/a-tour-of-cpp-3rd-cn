...
当一个类拥有指针成员时，通常最好显式定义拷贝和移动操作。原因是指针可能指向该类需要删除的东西，此时默认的逐成员拷贝会是错误的；或者它可能指向该类不得删除的东西。无论哪种情况，代码的读者都需要知道。示例见 [§6.2.1](6-2-copy-move.md#621-拷贝容器)。
...
接受单个参数的构造函数定义了从其参数类型到该类类型的转换。例如，`complex`（[§5.2.1](../ch05/5-2-concrete-types.md#521-算术类型)）提供了一个从 `double` 转换的构造函数：
...
这种隐式转换有时很理想，但并非总是如此。例如，`Vector`（[§5.2.2](../ch05/5-2-concrete-types.md#522-容器)）提供了一个从 `int` 转换的构造函数：
...
在定义类的数据成员时，我们可以提供一个默认初始化器，称为默认成员初始化器。考虑 `complex`（[§5.2.1](../ch05/5-2-concrete-types.md#521-算术类型)）的一个修订版：
