...
为了说明标准库容器，考虑一个用于保存姓名和电话号码的简单程序。对于这种程序，不同背景的人会认为不同的方法“简单而显而易见”。[§11.5](../ch11/11-5-user-defined-io.md) 中的 `Entry` 类可用于保存一个简单的电话簿条目。在这里，我们刻意忽略了许多现实世界的复杂性，例如许多电话号码不能用 32 位 `int` 简单表示。
...
`vector` 的元素构成一个范围，因此我们可以使用范围 `for` 循环（[§1.7](../ch01/1-7-pointers-arrays.md)）：

```cpp
void print_book(const vector<Entry>& book)
{
    for (const auto& x : book)   // 关于 "auto" 见 [§1.4](../ch01/1-4-types-variables.md)
        cout << x << '\n';
}
```
...
拷贝和移动 `vector` 是通过构造函数和赋值运算符实现的，如 [§6.2](../ch06/6-2-copy-move.md) 所述。赋值一个 `vector` 涉及拷贝其元素。因此，在 `book2` 初始化之后，`book2` 和 `phone_book` 各自持有电话簿中每个 `Entry` 的独立副本。当一个 `vector` 持有许多元素时，这种看起来无害的赋值和初始化可能代价高昂。在拷贝不可取的情况下，应使用引用或指针（[§1.7](../ch01/1-7-pointers-arrays.md)）或移动操作（[§6.2.2](../ch06/6-2-copy-move.md#622-移动容器)）。
...
`Vec` 从 `vector` 继承了所有内容，除了它重新定义了下标操作以进行范围检查。`at()` 操作是一个 `vector` 下标操作，如果其参数超出 `vector` 的范围，它会抛出 `out_of_range` 类型的异常（[§4.2](../ch04/4-2-exceptions.md)）。
...
异常将被抛出，然后被捕获（[§4.2](../ch04/4-2-exceptions.md)）。如果用户没有捕获异常，程序将以定义良好的方式终止，而不是以未定义的方式继续或失败。最小化未捕获异常带来的意外的一种方法是使用带有 `try` 块作为其主体的 `main()`。例如：
...
这提供了默认的异常处理程序，因此如果我们未能捕获某个异常，会在标准错误诊断输出流 `cerr` 上打印错误信息（[§11.2](../ch11/11-2-output.md)）。
...
当我们使用链表时，我们通常不像对 `vector` 那样使用下标来访问元素。相反，我们可能会搜索链表以查找具有给定值的元素。为此，我们利用 `list` 是一个序列的事实，如 [第 13 章](../ch13/index.md) 所述：
...
有时，我们需要标识 `list` 中的一个元素。例如，我们可能想要删除一个元素或在其之前插入一个新元素。为此，我们使用**迭代器**：`list` 迭代器标识 `list` 的一个元素，并可用于遍历 `list`（因此得名）。每个标准库容器都提供函数 `begin()` 和 `end()`，它们分别返回指向第一个元素和最后一个元素之后位置的迭代器（[§13.1](../ch13/13-1-introduction.md)）。显式使用迭代器，我们可以（不那么优雅地）这样写 `get_number()` 函数：
...
标准库 `map` 是一个键值对的容器，针对查找和插入进行了优化。我们可以使用与 `vector` 和 `list` 相同的初始化器（[§12.2](12-2-vector.md)，[§12.3](12-3-list.md)）：
...
如果我们想避免将无效号码输入电话簿，可以使用 `find()` 和 `insert()`（[§12.8](12-8-container-overview.md)）而不是 `[]`。
...
一个重要的长期运行系统使用了一个事件队列（见 [§18.4](../ch18/18-4-waiting-for-events.md)），将向量作为事件，以 `shared_ptr` 传递。这样，事件的最后一个使用者会隐式删除它：
...
void producer()
{
    for (int n = 0; n != LOTS; ++n) {
        lock_guard lk {m};               // m 是一个互斥量；见 [§18.3](../ch18/18-3-shared-data.md)
        q.push_back(make_shared<Event>());
        cv.notify_one();                 // cv 是一个条件变量；见 [§18.4](../ch18/18-4-waiting-for-events.md)
    }
}
```
...
{
    for (int n = 0; n != LOTS; ++n) {
        scoped_lock lk {m};                       // m 是一个互斥量（[§18.3](../ch18/18-3-shared-data.md)）
...
| 容器 | 描述 |
|------|------|
| `vector<T>` | 可变大小的向量（[§12.2](12-2-vector.md)） |
| `list<T>` | 双向链表（[§12.3](12-3-list.md)） |
| `forward_list<T>` | 单向链表 |
| `deque<T>` | 双端队列 |
| `map<K,V>` | 关联数组（[§12.5](12-5-map.md)） |
| `multimap<K,V>` | 一个键可以出现多次的 map |
| `unordered_map<K,V>` | 使用哈希查找的 map（[§12.6](12-6-unordered-map.md)） |
...
这些容器定义在命名空间 `std` 中，并在头文件 `<vector>`、`<list>`、`<map>` 等中提供（[§9.3.4](../ch09/9-3-standard-library-organization.md#934-头文件)）。此外，标准库还提供了容器适配器 `queue<T>`、`stack<T>` 和 `priority_queue<T>`。如果你需要它们，请查阅相关文档。标准库还提供了更专门的类似容器的类型，例如 `array<T,N>`（[§15.3.1](../ch15/15-3-containers.md#1531-array)）和 `bitset<N>`（[§15.3.2](../ch15/15-3-containers.md#1532-bitset)）。
